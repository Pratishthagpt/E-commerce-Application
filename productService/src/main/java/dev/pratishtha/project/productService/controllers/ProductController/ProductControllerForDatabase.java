...
@Tag(name = "Product management from database.")
@RestController
...
public class ProductControllerForDatabase {
    ...
    @Operation(summary = "API for getting all products sorted by title.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all products sorted by title.",
                    content = { @Content(mediaType = "application/json",
                            schema = @Schema(implementation = GenericProductDTO[].class))
                    })
    })
